...
class HelmetLoopAnimation extends StatefulWidget {
  const HelmetLoopAnimation({super.key});

  @override
  State<HelmetLoopAnimation> createState() => _HelmetLoopAnimationState();
}

class _HelmetLoopAnimationState extends State<HelmetLoopAnimation>
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < 9; i++) {
      ...
      final Paint speckPaint = Paint()
        ..color = kHazard.withValues(alpha: (kHazard.a * opacity).clamp(0, 1));
...
class _WavesPainter extends CustomPainter {
  final double waveOpacity;
  final double waveScale;
  _WavesPainter({required this.waveOpacity, required this.waveScale});
