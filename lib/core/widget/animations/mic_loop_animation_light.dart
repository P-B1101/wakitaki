...
// lib/ folder (or paste MicLoopAnimationLight into an existing app) and run.
//
...
final Color kSpeck = const Color(0xFF7A4600).withValues(alpha: 0.6);

const double kDurationSeconds = 4.5;
const double kCanvasSize = 800;

/// Square, seamlessly-looping mic animation. Sizes itself to fit whatever
/// bounded space it's given (wrap it in a fixed-size box, or SizedBox.expand
/// inside an AspectRatio(aspectRatio: 1) if you need a specific size).
class MicLoopAnimationLight extends StatefulWidget {
  const MicLoopAnimationLight({super.key});

  @override
  State<MicLoopAnimationLight> createState() => _MicLoopAnimationLightState();
}

class _MicLoopAnimationLightState extends State<MicLoopAnimationLight>
  ...
  Widget build(BuildContext context) {
    return AspectRatio(
      ...
      child: AnimatedBuilder(
        ...
        builder: (context, _) {
          return CustomPaint(
            painter: _MicLoopPainterLight(phase: _controller.value),
            size: Size.infinite,
          );
        },
      ),
...
class _MicLoopPainterLight extends CustomPainter {
  final double phase; // 0..1, wraps seamlessly

  _MicLoopPainterLight({required this.phase});
